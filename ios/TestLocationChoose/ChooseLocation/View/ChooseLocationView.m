...
- (void)addTableView{
   ...
   [tabbleView registerClass:[AddressTableViewCell class] forCellReuseIdentifier:@"AddressTableViewCell"];
}
...
- (NSIndexPath *)tableView:(UITableView *)tableView willSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if([self.tableViews indexOfObject:tableView] == 0){
        ...
        ProvinceModel * provinceItem = self.dataSouce[indexPath.row];
        self.cityDataSouce = [[CitiesDataTool sharedManager] QqueryAllRecordWithCityID:provinceItem.areaId];
        if(self.cityDataSouce.count == 0){
            for (int i = 0; i < self.tableViews.count && self.tableViews.count != 1; i++) {
                [self removeLastItem];
            }
            [self setUpAddress:provinceItem.areaName];
            return indexPath;
        }
        ...
        [self addTopBarItem];
        [self addTableView];
        ProvinceModel * item = self.dataSouce[indexPath.row];
        [self scrollToNextItem:item.areaName];
        
    }else if ([self.tableViews indexOfObject:tableView] == 1){
        
        ProvinceModel * cityItem = self.cityDataSouce[indexPath.row];
        self.districtDataSouce = [[CitiesDataTool sharedManager] QqueryAllRecordWithTownID:cityItem.areaId];
        NSIndexPath * indexPath0 = [tableView indexPathForSelectedRow];
        ...
        [self addTopBarItem];
        [self addTableView];
        ProvinceModel * item = self.cityDataSouce[indexPath.row];
        [self scrollToNextItem:item.areaName];
        
    }else if ([self.tableViews indexOfObject:tableView] == 2){
        
        ProvinceModel * item = self.districtDataSouce[indexPath.row];
        [self setUpAddress:item.areaName];
    }
    return indexPath;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if([self.tableViews indexOfObject:tableView] == 0){
       item = self.dataSouce[indexPath.row];
        ...
        NSDictionary *dict = [self setDict:item];
        [self.totalDict setObject:dict forKey:@"provinceModel"];
        
    }else if ([self.tableViews indexOfObject:tableView] == 1){
       item = self.cityDataSouce[indexPath.row];
        NSDictionary *dict = [self setDict:item];
        [self.totalDict setObject:dict forKey:@"cityModel"];
        
    }else if ([self.tableViews indexOfObject:tableView] == 2){
       item = self.districtDataSouce[indexPath.row];
        NSDictionary *dict = [self setDict:item];
        [self.totalDict setObject:dict forKey:@"TownModel"];
    }
    item.isSelected = YES;
...
-(NSDictionary *)setDict:(ProvinceModel *)model{
    NSMutableDictionary *dict = [[NSMutableDictionary alloc] init];
    [dict setObject:model.parentId forKey:@"pid"];
    [dict setObject:model.areaName forKey:@"name"];
    [dict setObject:model.areaId forKey:@"id"];
    return dict;
...
- (void)setAreaCode:(NSString *)areaCode{
    ...
    [self addTableView];
    NSError *error;
    NSData *data = [areaCode dataUsingEncoding:NSUTF8StringEncoding];
    NSArray * jsonObjectArr =[NSJSONSerialization JSONObjectWithData:data
                                                                   options:0
                                                                     error:&error];
    NSDictionary *ProvinceDict = jsonObjectArr[0];
    NSDictionary *CityDict = jsonObjectArr[1];
    NSDictionary *TownDict = jsonObjectArr[2];
    
    
    self.cityDataSouce = [[CitiesDataTool sharedManager] QqueryAllRecordWithCityID:[CityDict objectForKey:@"pid"]];
    self.districtDataSouce = [[CitiesDataTool sharedManager] QqueryAllRecordWithTownID:[TownDict objectForKey:@"pid"]];
   ...
   [midBtn setTitle:cityName forState:UIControlStateNormal];
     ...
     NSString * districtName = [TownDict objectForKey:@"name"];
...
- (void)setSelectedProvince:(NSString *)provinceName andCity:(NSString *)cityName andDistrict:(NSString *)districtName {
    ...
    for (ProvinceModel * item in self.dataSouce) {
        if ([item.areaName isEqualToString:provinceName]) {
    ...
    for (int i = 0; i < self.cityDataSouce.count; i++) {
        ProvinceModel * item = self.cityDataSouce[i];
        
        if ([item.areaName isEqualToString:cityName]) {
    ...
    for (int i = 0; i <self.districtDataSouce.count; i++) {
        ProvinceModel * item = self.districtDataSouce[i];
        if ([item.areaName isEqualToString:districtName]) {
            NSIndexPath * indexPath = [NSIndexPath indexPathForRow:i inSection:0];
...
- (NSArray *)dataSouce{
    ...
    if (_dataSouce == nil) {
        ...
        _dataSouce = [[CitiesDataTool sharedManager] QqueryAllRecordWithProviceID:@0];
    }
    return _dataSouce;
}
//返回数据数组
